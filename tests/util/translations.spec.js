  ...
  describe('getIntlIds', function () {
    ...
    beforeEach(function () {
      context = {
        ...
        settings: {
          localeFiles: [
            'tests/mocks/locale_a.json',
            'tests/mocks/locale_b.json'
          ]
        }
      };
    });
    ...
    it('throws exception if localeFiles are not provided', function () {
      ...
      expect(() => {
        translations.getIntlIds(context);
      }).toThrow();
    });
    it('returns nested translation keys', function () {
      context = {
        id: 'id-missing',
        settings: {
          localeFiles: ['tests/mocks/nested.json']
        }
      };
      const keys = translations.getIntlIds(context);
      expect(keys).toBeInstanceOf(Array);
